...
├── docker/                 # Nginx and PHP-FPM configs
├── Dockerfile              # Multi-stage production build
└── docker-compose.yml      # Full stack (app + MySQL + Redis)
```
...
### Option 3: AI-powered setup (Cursor)
...
The AI agent will install prerequisites, configure everything, and start the server automatically.
...
|---------|------|-------------|
| **app** | 8080 | Laravel + Nginx (serves the full app) |
| **mysql** | 3306 | MySQL 8 database |
| **redis** | 6379 | Cache and queue driver |

### Environment Variables

For production, override these in `.env` or pass via `docker compose`:
...
| `APP_URL` | `http://localhost` | Your production URL |
| `DB_CONNECTION` | `sqlite` | Use `mysql` for Docker/production |
| `DB_HOST` | `127.0.0.1` | Use `mysql` (service name) in Docker |
| `DB_DATABASE` | `timegrid` | Database name |
| `DB_USERNAME` | `timegrid` | Database user |
